...
!
     ...
     &               (model.eq.13)) THEN
!
!  Read in time-evolving ice draft (m).
!
        IF (get_var(iddraft)) THEN
          foundit=find_string(var_name, n_var, TRIM(Vname(1,iddraft)),  &
     &                        varid)
          gtype=var_flag(varid)*r2dvar
          status=nf_fread2d(ng, IDmod, ncname, ncINPid,                 &
     &                      Vname(1,iddraft), varid,                    &
     &                      InpRec, gtype, Vsize,                       &
     &                      LBi, UBi, LBj, UBj,                         &
     &                      Fscl, Fmin, Fmax,                           &
     &                      GRID(ng) % rmask,                           &
     &                      GRID(ng) % zice)
          IF (status.ne.nf90_noerr) THEN
            IF (Master) THEN
              WRITE (stdout,60) string, TRIM(Vname(1,iddraft)), InpRec, &
     &                          TRIM(ncname)
            END IF
            exit_flag=2
            ioerror=status
          ...
          ELSE
            IF (Master) THEN
              WRITE (stdout,70) TRIM(Vname(2,iddraft)), Fmin, Fmax
            END IF
          END IF
        END IF
!
